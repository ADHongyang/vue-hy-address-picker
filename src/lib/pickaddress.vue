<style>
  ...
  .hyAddresspick-content .first{
    text-align: center;
    margin: 0 auto;
    padding: 8px 0;
    position: relative;
  }
  .hyAddresspick-content .first img{
    position: absolute;
    right:12px;
    top: 8px;
  }
  .hyAddresspick-content .second{
    border-top: 1px solid #eeeeee;
    border-bottom: 1px solid #eeeeee;
    padding: 8px 8px;
    font-size: 12px;
  }
  .hyAddresspick-content .second span{
    display: inline-block;
    padding: 0 3px;
  }
  .hyAddresspick-content .second span.on{
    color: red;
  }
  .hyAddresspick-content .third{
    height: 160px;
    overflow: auto;
    -webkit-overflow-scrolling: touch;
  }
  .hyAddresspick-content .third div{
    margin: 6px 8px;
  }
  .hyAddresspick-content .third div.on{
    color: red;
  }
  ...
  .dis-enter-active, .dis-leave-active {
    transition: bottom .3s;
  }
  .dis-enter, .dis-leave-to {
    bottom: -300px;
  }
</style>

<template>
    <div class="hyAddresspick" id="hyAddresspick" :style="{height: height}">
      <transition name="fade">
        <div class="hyAddresspick-mask" v-if="show" @click="show=false"></div>
      </transition>
      <transition name="dis">
        <div class="hyAddresspick-content" v-if="show">
        <div class="first" >
          请选择
          <img src="./clear1.png" width="17" height="17" alt="" @click="show=false">
        </div>
        <div class="second">
          <span :class="{on: onwho=='sheng'}" @click="shengparse">{{shengdata.show}}</span>
          <span :class="{on: onwho=='shi'}" @click="shiparse">{{shidata.show}}</span>
          <span :class="{on: onwho=='qu'}" @click="quselect">{{qudata.show}}</span>
        </div>
        <div class="third">
          <div :class="{on: onwhich==data.name}" v-for="data in datadetail" @click="pickwho(data.name)">{{data.name}}</div>
        </div>
      </div>
      </transition>
    </div>
...
//             {name: '厦门市', index: 1},
    ...
    methods: {
        initfunc: function() {
          ...
          self.show = true;
          if(!self.city) {//空
              self.shengdata.name = '请选择';
              self.shengdata.show = '请选择';
              self.shengselect();
              return;
          }else {
//              console.log(self.city)
              try{
                  var obj = JSON.parse(self.city);
              }catch (err) {
                  console.log('请传送正确的json字符串');
                  self.shengdata.name = '请选择';
                  self.shengdata.show = '请选择';
                  self.shengselect();
                  return;
              }
//              if(self.nocity(self.shengdata.name)) {//直辖市，港澳台
//
//              }else {
                  self.shengdata.name = self.shengdata.show = obj.province;
                  self.shidata.name = self.shidata.show = obj.city;
                  self.qudata.name = self.qudata.show = obj.area;
                  self.quselect();
//              }
          }
        ...
        shengparse: function() {
            var self = this;
            self.onwho = 'sheng';//选择时显示红色
            self.datadetail = [];
            for(var index in self.cities) {
                var obj = {};
                obj.name = index;
                self.datadetail.push(obj);
            }
            self.onwhich = self.shengdata.name;//如果已选择，选中的显示红色
        },
        shiselect: function() {
            var self = this;
            self.shiparse();
        ...
        shiparse: function() {
            ...
            if(self.nocity(self.shengdata.name)) {//直辖市，港澳台
                ...
                self.onwho = 'shi';//选择时显示红色
                for(var index in self.cities[self.shengdata.name]) {
                    var obj = {};
                    obj.name = index;
                    self.datadetail.push(obj);
                }
                self.onwhich = self.shidata.name;//如果已选择，选中的显示红色
            }

        },
        //检查是否为直辖市，港澳台
        nocity: function(str) {
          var self = this, checkstring = '台湾香港澳门重庆上海北京天津';
          return checkstring.indexOf(str) > -1 ? true : false;
        },
        quselect: function() {
            var self = this;
            self.onwho = 'qu';
            self.datadetail = [];
            if(self.nocity(self.shengdata.name)) {//直辖市，港澳台
                self.shidata.name = self.shengdata.name;
                self.shidata.show = '';
            }
            var arr = self.cities[self.shengdata.name][self.shidata.name];
            for(var index in arr) {
                var obj = {};
                obj.name = arr[index];
                self.datadetail.push(obj);
            }
            self.onwhich = self.qudata.name;
        },
        pickwho: function(who) {
            var self = this;
          if(self.onwho == 'sheng') {
              self.shengdata.name = who;
              self.shengdata.show = who;
              self.shidata.name = '请选择';
              self.shidata.show = '请选择';
              self.shiselect();//开始选择市
          }else if(self.onwho == 'shi') {
              self.shidata.name = who;
              self.shidata.show = who;
              self.qudata.name = '请选择';
              self.qudata.show = '请选择';
              self.quselect();//开始选择区
          }else {
              self.qudata.name = who;
              self.qudata.show = who;
              self.onwhich = who;
//              console.log(who)
              //保存数据
              var obj = {};
              obj.province = self.shengdata.name;
              obj.city = self.shidata.name;
              obj.area = self.qudata.name;
              //向父组件发送数据
              self.$emit('hypickaddress', JSON.stringify(obj));
              self.show = false;
          }
        },
    },
